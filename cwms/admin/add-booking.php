...
if(strlen($_SESSION['alogin']) == 0) {	
    ...
    if(isset($_POST['book'])) {
        ...
        $conflict_sql = "SELECT id FROM tblbooking WHERE carWashPoint = :wpoint AND washDate = :date AND washTime = :time";
        $conflict_query = $dbh->prepare($conflict_sql);
        $conflict_query->bindParam(':wpoint', $wpoint, PDO::PARAM_STR);
        $conflict_query->bindParam(':date', $date, PDO::PARAM_STR);
        $conflict_query->bindParam(':time', $time, PDO::PARAM_STR);
        $conflict_query->execute();

        if($conflict_query->rowCount() > 0) {
            echo "<script>alert('Booking already exists for this washing point, date, and time. Please choose a different slot.');</script>";
        } else {
            // Proceed with booking
            $sql = "INSERT INTO tblbooking(bookingId, packageType, carWashPoint, fullName, mobileNumber, washDate, washTime, message, status)
            ...
            if($lastInsertId) {
                echo "<script>alert('Booking successfully added. Booking number is $bno');</script>";
  	       ...
  	       <h3>Add Car Washing Booking</h3>
  	         ...
  	         <div class="tab-content">
                ...
                <option value="1">BASIC CLEANING (₹1000)</option>
                 <option value="2">PREMIUM CLEANING (₹1800)</option>
                  <option value="3 ">COMPLEX CLEANING(₹2700)</option>
              </select>
									</div>
								</div>
<div class="form-group">
